...
export default function DashboardLayout({ children }) {
  ...
  if (checking) {
    return (
      <div className="min-h-screen grid place-items-center bg-slate-100">
        <div className="portal-shell px-5 py-3 text-sm text-slate-600">
          Loading…
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-900">
      <aside className="hidden md:flex md:flex-col fixed left-0 top-0 w-72 h-screen z-40 border-r border-slate-200 bg-white">
        <div className="relative px-6 py-6 border-b border-slate-200">
          ...
          <p className="mt-4 text-[11px] uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2 font-semibold">
            ...
            return (
              <Link
                ...
                className={
                  ...
                  (active
                    ? "text-[#0f3d7a] bg-[#eef4fd] border-[#c9d9ef]"
                    : "text-slate-700 hover:text-slate-900 hover:bg-slate-50 border-transparent hover:border-slate-200")
                }
              >
                <span className={
                  "inline-flex min-w-8 justify-center rounded-md px-1.5 py-0.5 text-[11px] font-semibold " +
                  (active ? "bg-[#0f3d7a] text-white" : "bg-slate-200 text-slate-600")
                }>{item.icon}</span>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5 border-t border-slate-200 bg-white">
          <button
            onClick={async () => {
              await signOut(auth);
              router.push("/login");
          ...
          <p className="mt-3 text-xs text-slate-500 text-center font-medium">Portal v0.0.1</p>
        </div>
      </aside>
