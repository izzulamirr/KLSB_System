...
export async function readInvoiceRows(options = {}) {
  ...
  const local = await readLocalInvoiceRows();
  if (local.found) return local.rows;
...
// Returns { rows, found }. `found: false` means the store has never been
// initialized (or is corrupt) and `rows` is just the seed fallback — callers
// use this to tell that apart from a real, intentionally empty rows array.
async function readLocalInvoiceRows() {
  ...
  try {
    ...
    if (Array.isArray(parsed)) return { rows: parsed.map(normalizeInvoiceRow), found: true };
    if (Array.isArray(parsed?.rows)) return { rows: parsed.rows.map(normalizeInvoiceRow), found: true };
  } catch (error) {
    if (error?.code !== "ENOENT") {
      console.error(`Failed to read invoice store at ${path}, falling back to seed data`, error);
    }
    return { rows: getSeedInvoiceRows(), found: false };
  }
  return { rows: getSeedInvoiceRows(), found: false };
}
...
async function writeLocalInvoiceRows(rows) {
  ...
  async function performWrite() {
    ...
    await fs.writeFile(tempPath, JSON.stringify({ rows: normalized, updatedAt: new Date().toISOString() }, null, 2), "utf8");
    await fs.rename(tempPath, path);
    return normalized;
  }

  // Serialize writes within this process so two concurrent saves can't
  // interleave; the temp-file-then-rename above keeps each write atomic.
...
function isFinanceRelevant(row) {
  // Drafts haven't been sent to the client yet, so they aren't real
  // receivables — only count an invoice once it's left draft.
...
export function buildFinanceData(rows) {
  ...
  function outstandingAmount(row) {
    return Math.max(parseAmount(row.invoiceAmountWithSst || row.invoiceAmountWoSst) - parseAmount(row.paidAmount), 0);
  }
  ...
  const overdueAmount = overdueRows.reduce((sum, row) => sum + outstandingAmount(row), 0);

  const paidThisMonthTotal = paidInvoices.reduce((sum, row) => {
    const paidOn = new Date(row.paymentDate);
    if (Number.isNaN(paidOn.getTime()) || paidOn.getMonth() !== now.getMonth() || paidOn.getFullYear() !== now.getFullYear()) {
      return sum;
    }
    return sum + parseAmount(row.paidAmount || row.invoiceAmountWithSst);
  }, 0);

  const collectionDurations = paidInvoices.reduce((days, row) => {
    const issued = new Date(row.dateIssued);
    const paidOn = new Date(row.paymentDate);
    if (Number.isNaN(issued.getTime()) || Number.isNaN(paidOn.getTime())) return days;
    const span = Math.round((paidOn.getTime() - issued.getTime()) / (24 * 60 * 60 * 1000));
    if (span < 0) return days;
    days.push(span);
    return days;
  }, []);
  const avgCollectionDays = collectionDurations.length
    ? Math.round(collectionDurations.reduce((sum, span) => sum + span, 0) / collectionDurations.length)
    : 0;
  ...
  const forecastInvoices = openInvoices.filter((row) => {
    const dueDate = new Date(row.dueDate);
    if (Number.isNaN(dueDate.getTime())) return false;
    return dueDate >= now && dueDate <= new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000);
  });
  const forecastInflow = forecastInvoices.reduce((sum, row) => sum + outstandingAmount(row), 0);

  const agingCurrentRows = openInvoices.filter((row) => {
    const days = daysOverdue(row);
    return days == null || days <= 30;
  });
  const aging31to60Rows = overdueRows.filter((row) => daysOverdue(row) > 30 && daysOverdue(row) <= 60);
  const aging61to90Rows = overdueRows.filter((row) => daysOverdue(row) > 60 && daysOverdue(row) <= 90);
  const aging90PlusRows = overdueRows.filter((row) => daysOverdue(row) > 90);

  return {
    stats: [
      { label: "Total Invoices", value: String(invoiceCount), note: "All invoice records" },
      { label: "Paid This Month", value: formatCurrency(paidThisMonthTotal), note: "Computed from invoice data" },
      ...
      { label: "Avg Collection Days", value: `${avgCollectionDays} days`, note: "Derived from paid invoice dates" },
      { label: "Forecast Inflow", value: formatCurrency(forecastInflow), note: "Next 30 days" },
    ],
    kpiBreakdown: {
      ...
      dso: String(avgCollectionDays),
    },
    agingBuckets: [
      { bucket: "Current (0-30)", amount: formatCurrency(agingCurrentRows.reduce((sum, row) => sum + outstandingAmount(row), 0)), invoiceCount: agingCurrentRows.length },
      { bucket: "31-60 Days", amount: formatCurrency(aging31to60Rows.reduce((sum, row) => sum + outstandingAmount(row), 0)), invoiceCount: aging31to60Rows.length },
      { bucket: "61-90 Days", amount: formatCurrency(aging61to90Rows.reduce((sum, row) => sum + outstandingAmount(row), 0)), invoiceCount: aging61to90Rows.length },
      { bucket: "90+ Days", amount: formatCurrency(aging90PlusRows.reduce((sum, row) => sum + outstandingAmount(row), 0)), invoiceCount: aging90PlusRows.length },
    ],
    ...
    receivables,
    recentInvoices,
    recentPayments: paidInvoices.slice(0, 5).map((row) => ({
      reference: row.poSoNumber || "-",
      client: row.clientName,
