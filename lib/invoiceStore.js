...
export async function readInvoiceRows() {
...
function isPaid(row) {
  return String(row?.status || "").toLowerCase().includes("paid") || Number(row?.sent) === 1 || row?.sent === true;
}

export function buildFinanceData(rows) {
  const invoices = Array.isArray(rows) ? rows.map(normalizeInvoiceRow) : [];
  const invoiceCount = invoices.length;
  const paidInvoices = invoices.filter((row) => isPaid(row));
  const openInvoices = invoices.filter((row) => !isPaid(row));

  const billedTotal = invoices.reduce((sum, row) => sum + parseAmount(row.invoiceAmountWithSst || row.invoiceAmountWoSst), 0);
  ...
  const recentInvoices = [...invoices]
    ...
    .map((row) => ({
      ...
      po: row.poSoNumber || "-",
      status: String(row.status || (row.sent ? "Sent" : "Draft")),
    }));
  ...
  const receivables = openInvoices
    ...
    .map((row) => ({
      ...
      status: new Date(row.dueDate || 0) < new Date() ? "Overdue" : "Open",
    }));
  ...
  const currentInvoices = invoices.filter((row) => {
    const dueDate = new Date(row.dueDate);
