...
// Public: read all manpower records from Firestore collection `manpower`
export async function getManpowerData({ limit = 1000 } = {}) {
  const admin = await initAdmin();
  const db = admin.firestore();
  const col = db.collection("manpower");
  const q = limit ? col.limit(limit) : col;
