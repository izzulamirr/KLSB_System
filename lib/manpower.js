...
export async function getManpowerData({ limit = 1000, bil = null, poSoNo = null, location = null } = {}) {
  const admin = await initAdmin();
  const db = admin.firestore();
  let q = db.collection("manpower");

  if (bil !== null && bil !== undefined && String(bil).trim() !== "") {
    const asNum = Number(bil);
    q = q.where("BIL", "==", Number.isNaN(asNum) ? bil : asNum);
  } else if (poSoNo) {
...
export async function importCsvToFirestore(filePath, options = {}) {
  ...
  for (let i = 0; i < docs.length; i += opts.batchSize) {
    ...
    for (const doc of chunk) {
      // Use an auto-id; you can change to upsert by using a deterministic ID
      const id = doc.BIL ? String(doc.BIL) : undefined;
